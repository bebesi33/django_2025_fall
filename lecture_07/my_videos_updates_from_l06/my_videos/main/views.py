from django.shortcuts import get_object_or_404, redirect, render
import logging

from django.urls import reverse_lazy
from django_tables2 import RequestConfig

from main.forms import ModifyVideoForm, VideoForm, check_video_existence
from main.filters import VideoFilter
from main.models import Video
from main.tables import VideoTable
from django.views.generic.edit import CreateView

logger = logging.getLogger(__name__)


# Create your views here.
def index(request):
    index_context = dict()
    return render(request, "index.html", index_context)


def error_404_view(request, exception):
    logger.error(f"Hibák vannak és lesznek: {exception}")
    return render(request, "error_404.html", status=404)


def show_videos(request):
    queryset = Video.objects.all()
    blog_filter = VideoFilter(request.GET, queryset=queryset)
    blog_table = VideoTable(blog_filter.qs)
    RequestConfig(request, paginate={"per_page": 5}).configure(blog_table)
    # erre itt találtok példákat: https://django-tables2.readthedocs.io/en/latest/pages/pagination.html
    return render(
        request, "video_view.html", {"table": blog_table, "filter": blog_filter}
    )


def add_video(request):
    if request.method == "POST":
        form = VideoForm(request.POST)
        print(f"Form status: {form.is_bound}")
        if form.is_valid():
            form.save()
            return redirect("videos")
        else:
            print(f"Form errors: {form.errors}")
    else:
        form = VideoForm()
    return render(request, "add_video.html", {"form": form})


def get_video_details(request, id):
    video = get_object_or_404(Video, pk=id)
    orig_title = video.title
    orig_url = video.url
    if request.method == "POST":
        form = ModifyVideoForm(
            request.POST, instance=video, orig_title=orig_title, orig_url=orig_url
        )
        if form.is_valid():
            form.save()
            return redirect("videos")
    elif request.method == "GET":
        form = ModifyVideoForm(
            data=None, instance=video, orig_title=orig_title, orig_url=orig_url
        )
    return render(request, "video_detail_view.html", {"video": video, "form": form})

def video_delete_view(request, id):
    """Rövid , nem túl korrekt verzió"""
    video = get_object_or_404(Video, pk=id)
    if request.method == "POST":
        video.delete()
        return redirect("videos")
    return render(request, "video_confirm_delete.html", {"video": video})


class VideoCreateView(CreateView):
    model = Video
    form_class = VideoForm
    template_name = "add_video.html"
    succes_url = reverse_lazy("videos")
